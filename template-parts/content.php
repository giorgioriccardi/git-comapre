    ...
    if (is_singular()) {
        ...
        if ( 'no-image' != $image_option ){
            if (has_post_thumbnail()) {
                ...
                $show_read_time = minimal_lite_get_option('show_read_time_archive_pages', true);
                $show_read_words = minimal_lite_get_option('number_of_words_per_min', '150');
                if( true == ($show_read_time) ){
                    $content = apply_filters( 'the_content', get_the_content() );
                    $word_count = minimal_lite_count_content_words( $content );
                    //echo $word_count;
                    $word_per_min = ($word_count / $show_read_words);
                    $unit_time = __(' minute read','minimal-lite');
                    echo "<span class = 'read-time'> <i class='ion-android-alarm-clock'></i>";
